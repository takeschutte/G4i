...
Require Import G4i.Substitutions.
Require Import Stdlib.Program.Equality.

(* Note 3 or 4 would suffice for IPC ; iSL requires 5 *)
Definition env_weight Γ := list_sum (map (fun x => 5^ weight x) Γ).
...
Lemma env_weight_subst Γ (f : nat -> term):
  env_weight (map (subst_form f) Γ) = env_weight Γ.
Proof.
  induction Γ.
  - easy.
  - simpl.
    pose (weight_subst_f f a) as Hw1.
    simpl in Hw1.
...
Global Hint Resolve env_weight_subst : order.
...
Lemma pow5_a_lt_b a b : (a < b) -> (5 ^ a < 5 ^ b).
Proof. intros. apply Nat.pow_lt_mono_r; lia. Qed.

Hint Resolve pow5_a_lt_b : order.
...
Lemma env_weight_elements_subst {Γ : env} f :
  env_weight (elements (gmultiset_map (subst_form f) Γ)) = env_weight (elements Γ).
Proof.
  induction Γ as [|z Z IH] using gmultiset_ind.
  - easy.
  - replace ({[+ z +]} ⊎ Z) with (Z ⊎ {[+ z +]}) by multiset_solver.
    rewrite (gmultiset_map_distr_disj_union_singleton (subst_form f) Z z).
    rewrite !elements_env_add.
    rewrite !env_weight_add.
    pose (weight_subst_f f z) as Hw1.
    simpl in Hw1.
    rewrite Hw1.
    rewrite <- IH.
    reflexivity.
Qed.
Global Hint Resolve  env_weight_elements_subst : order.
